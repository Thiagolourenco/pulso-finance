-- ============================================
-- Script SQL SEGURO para criar todas as tabelas do Pulso
-- Versão sem operações DROP - mais segura para executar
...
CREATE INDEX IF NOT EXISTS idx_card_purchases_is_paid_current_month ON card_purchases(is_paid_current_month);
...
CREATE INDEX IF NOT EXISTS idx_recurring_expenses_is_paid_current_month ON recurring_expenses(is_paid_current_month);
...
-- Triggers para atualizar updated_at (cria apenas se não existir)
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_card_purchases_updated_at') THEN
    CREATE TRIGGER update_card_purchases_updated_at BEFORE UPDATE ON card_purchases
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_card_invoices_updated_at') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_recurring_expenses_updated_at') THEN
...
-- Policy: categories (cria apenas se não existir)
DO $$
BEGIN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cards' AND policyname = 'Users can view their own cards') THEN
    CREATE POLICY "Users can view their own cards"
      ON cards FOR SELECT
      USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cards' AND policyname = 'Users can insert their own cards') THEN
    CREATE POLICY "Users can insert their own cards"
      ON cards FOR INSERT
      WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cards' AND policyname = 'Users can update their own cards') THEN
    CREATE POLICY "Users can update their own cards"
      ON cards FOR UPDATE
      USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cards' AND policyname = 'Users can delete their own cards') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_purchases' AND policyname = 'Users can view their own card purchases') THEN
    CREATE POLICY "Users can view their own card purchases"
      ON card_purchases FOR SELECT
      USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_purchases' AND policyname = 'Users can insert their own card purchases') THEN
    CREATE POLICY "Users can insert their own card purchases"
      ON card_purchases FOR INSERT
      WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_purchases' AND policyname = 'Users can update their own card purchases') THEN
    CREATE POLICY "Users can update their own card purchases"
      ON card_purchases FOR UPDATE
      USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_purchases' AND policyname = 'Users can delete their own card purchases') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_invoices' AND policyname = 'Users can view their own card invoices') THEN
    CREATE POLICY "Users can view their own card invoices"
      ON card_invoices FOR SELECT
      USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_invoices' AND policyname = 'Users can insert their own card invoices') THEN
    CREATE POLICY "Users can insert their own card invoices"
      ON card_invoices FOR INSERT
      WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_invoices' AND policyname = 'Users can update their own card invoices') THEN
    CREATE POLICY "Users can update their own card invoices"
      ON card_invoices FOR UPDATE
      USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'card_invoices' AND policyname = 'Users can delete their own card invoices') THEN
